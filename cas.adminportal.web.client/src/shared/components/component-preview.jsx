...
const ComponentPreviewContext = React.createContext(null);

export function useComponentPreview() {
  const context = React.useContext(ComponentPreviewContext);
  if (!context) {
    // If there's no provider in the tree, return safe defaults so
    // components that optionally read preview state (e.g. rtl) can
    // operate outside of the preview UI without crashing.
    return {
      path: "",
      code: "",
      highlightedCode: "",
