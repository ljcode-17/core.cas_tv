...
export default plugin(({
    ...
    const getContextualColors = (mode, themeColor) => {
        const contextual = theme('base.colors.contextual')[mode];
        const vars = {};

        const colorGroups = contextual[themeColor];

        for (const variant of Object.keys(colorGroups)) {
            const states = colorGroups[variant];

            for (const state of Object.keys(states)) {
                const cssVar = `--tw-${variant}${state === 'default' ? '' : '-' + state}`;
                vars[cssVar] = states[state];
            }
        }
    ...
    const getDefaultContextualColors = (mode) => {
        ...
        const redGroup = contextual['red'];

        for (const variant of Object.keys(redGroup)) {
            for (const state of Object.keys(redGroup[variant])) {
                const color = redGroup[variant][state];
    ...
    const getThemeColorDarkMode = () => {
        const themeDarkMode = theme('colors.themeColorDarkMode');
        const vars = {};

        for (const variant of Object.keys(themeDarkMode)) {
            vars[`--tw-themeColorDarkMode-${variant}`] = themeDarkMode[variant];
        }
    ...
    supportedThemeColors.forEach((themeColor) => {
        addBase({
            [`.theme-${themeColor}.light`]: {
                ...getGrayColors('light'),
                ...getContextualColors('light', themeColor),
                ...getContextualBoxShadows('light'),
                ...getComponentBackgroundColors('light'),
                ...getComponentBorders('light'),
                ...getComponentsBoxShadows('light'),
            ...
            [`.theme-${themeColor}.dark`]: {
                ...getGrayColors('dark'),
                ...getContextualColors('dark', themeColor),
                ...getContextualBoxShadows('dark'),
                ...getComponentBackgroundColors('dark'),
                ...getComponentBorders('dark'),
                ...getComponentsBoxShadows('dark'),
